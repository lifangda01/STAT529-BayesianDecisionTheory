...
def find_best_gen_beta_fit(hist_data):
	...
	for alpha in linspace(0.1,10,100):
		for beta in linspace(0.1,10,100):
			cdf = cumsum( get_gen_beta_pdf(linspace(-0.5,1.5,20), alpha, beta, -0.5, 1.5) ) * 0.1
			# Restrict on [0,1]
	...
	cdf = cumsum( get_gen_beta_pdf(linspace(-0.5,1.5,20), best_alpha, best_beta, -0.5, 1.5) ) * 0.1
	cdf = cdf[4:14] / (cdf[14] - cdf[4])
	plot( cdf , label='fit')
	legend(loc=4)
	show()
...
def find_bayes_interval(alpha, beta):
	...
	'''
	cdf = cumsum( get_gen_beta_pdf(linspace(-0.5,1.5,200), alpha, beta, -0.5, 1.5) ) * 0.01
	best_a, best_b = 0, 0
	best_loss = 999
